...
class FFAFieldMapper():
	...
	def field_map_cylindrical(self, axes = None):
		...
		axes: matplotlib Axes object or None 
			If defined, plot the field map on axes as a hist2d; if None make a 
			new figure/axes and plot it there.
	...
	def field_map_cartesian(self, radius):
		...
		if not axes:
			...
			axes = figure.add_subplot(1, 1, 1)
		min_by, max_by, cmax = self.gen_cmap(bz_grid)
		hist = axes.hist2d(x_grid, y_grid, bins=[x_bins, y_bins], weights=bz_grid, cmin=min_by, cmax=max_by, cmap=self.cmap, vmin=-cmax, vmax=cmax)
